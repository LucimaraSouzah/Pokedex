/*
    HTML
        RenderCard(obj)                             Adiciona 1 card no html
        RenderCardList(array_obj)                   Adiciona uma lista de cards
        ...
        RenderModal(id)                             Renderiza o modal
        RenderPage(pg)                              Carrega mais 30 items e renderiza
        RenderNames(array_names)                    Renderiza os personagens
        RenderNamesPage(array_names, num)           Renderiza os personagens de acordo as páginas
...
function init(){
    ...
    // PokemonsInfoPage(0).then(e=>RenderCardList(e))
    // RenderPage(Pokemon.Page)
    // AddPokemonsEndScreen()
...
function RenderCard(obj, category){
...
function ObjectAtt(obj, name){
    // retorna o attribute do objeto passado pelo nome
...
async function PokemonsInfo(id){
    ...
    obj = {
        ...
        weight: j.weight,
        height: j.height,
        status: j.stats.map(e=>e.base_stat) // hp, atk, def, satk, sdef, speed
    }
